...
class Semaphore {
 ...
 public:
  ...
  bool timed_wait(std::uint64_t usecs) { return WaitForSingleObject(m_hSema, (unsigned long)(usecs / 1000)) == 0; }

  void signal(int count = 1) { while (!ReleaseSemaphore(m_hSema, count, nullptr)); }
};
#elif defined(__MACH__)
//---------------------------------------------------------
...
class Semaphore {
 ...
 public:
  ...
  void signal() { while (semaphore_signal(m_sema) != KERN_SUCCESS); }
...
class Semaphore {
 ...
 public:
  ...
  void signal() { while (sem_post(&m_sema) == -1); }
...
class LightweightSemaphore {
 ...
 private:
  ...
  bool waitWithPartialSpinning(std::int64_t timeout_usecs = -1) {
    ...
    while (--spin >= 0) {
      ...
      if ((oldCount > 0) &&
          m_count.compare_exchange_strong(oldCount, oldCount - 1, std::memory_order_acquire, std::memory_order_relaxed))
        return true;
    ...
    if (timeout_usecs > 0 && m_sema.timed_wait((std::uint64_t)timeout_usecs)) return true;
    ...
    while (true) {
      ...
      if (oldCount < 0 &&
          m_count.compare_exchange_strong(oldCount, oldCount + 1, std::memory_order_relaxed, std::memory_order_relaxed))
        return false;
    }
  }
 ...
 public:
  ...
  ssize_t waitMany(ssize_t max, std::int64_t timeout_usecs) {
    ...
    if (result == 0 && max > 0) result = waitManyWithPartialSpinning(max, timeout_usecs);
    return result;
  }
...
}  // end namespace ncaf::embedded_3rd::moodycamel
